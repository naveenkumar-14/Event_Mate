...
const AddEvent = () => {
  ...
  // ✅ Fetch static data (cities, music, photographers)
  useEffect(() => {
    Promise.all([
      fetch("http://localhost:8080/venue/cities").then((r) => r.json()),
      fetch("http://localhost:8080/musicsystem/all").then((r) => r.json()),
      fetch("http://localhost:8080/photographer/all").then((r) => r.json()),
    ])
      .then(([cityData, musicData, photoData]) => {
        setCities(cityData);
        setMusicList(musicData);
        setPhotographersList(photoData);
      })
      .catch((err) => console.error("Error fetching static lists:", err));
  }, []);

  // ✅ Memoized filtered lists (avoids flicker)
  const filteredMusic = useMemo(() => {
    return musicList.filter(
      (m) => m.musicSystemCity?.toLowerCase() === selectedCity?.toLowerCase()
    );
  }, [musicList, selectedCity]);

  const suggestedPhotographers = useMemo(() => {
    return photographersList.filter(
      (p) => p.photographerCity?.toLowerCase() === selectedCity?.toLowerCase()
    );
  }, [photographersList, selectedCity]);

  // ✅ Fetch venues only when city changes
  useEffect(() => {
    if (!selectedCity) return;
    fetch(`http://localhost:8080/venue/list/${selectedCity}`)
      .then((res) => res.json())
      .then((data) => setSuggestedVenues(data))
      .catch((err) => console.error("Error fetching venues:", err));
  }, [selectedCity]);
  ...
  const handleSelectVenue = (v) => {
    setSelectedVenue(v);
    setFormData((p) => ({ ...p, eventVenue: v.venueName }));
  };

  const handleSelectMusic = (m) => {
    setSelectedMusic(m);
    setFormData((p) => ({ ...p, eventMusicSystem: m.musicSystemName }));
  };

  const handleSelectPhotographer = (p) => {
    setSelectedPhotographer(p);
    setFormData((p) => ({ ...p, eventPhotographer: p.photographerName }));
  };
  ...
  return (
    <div className="add-event" style={{ position: "relative", zIndex: 1 }}>
      <h2>
        {editData ? "Update Event" : "Add Event"} – {service?.title || `Service ${id}`}
      </h2>
      ...
      <form className="event-form" onSubmit={handleSubmit}>
        ...
        {activeTab === "venue" && (
          <div className="venue-section" style={{ position: "relative", zIndex: 10 }}>
            <label>Select City</label>
            <select
              value={selectedCity}
              onChange={(e) => {
              ...
              {suggestedPhotographers.map((p) => (
                <div className="venue-card" key={p.photographerId}>
                  ...
                  <button
                    type="button"
                    disabled={
                      selectedPhotographer?.photographerId === p.photographerId
                    }
                    className={
                      selectedPhotographer?.photographerId === p.photographerId
                        ? "selected-btn"
